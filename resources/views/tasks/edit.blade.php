            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('tasks.update', $task) }}">
                        ...
                        @if($task->project)
                            <div class="mb-4">
                                <x-input-label for="assigned_to" :value="__('Assign To (Project Member)')" />
                                <select id="assigned_to" name="assigned_to" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    ...
                                    @endforeach
                                    <!-- Add Owner if not in users pivot (though owner should be in pivot) -->
                                </select>
                                <x-input-error :messages="$errors->get('assigned_to')" class="mt-2" />
                            </div>
                        @else
                            <!-- Hidden Project ID if we want to allow moving tasks between projects, but keeping simple for now -->
